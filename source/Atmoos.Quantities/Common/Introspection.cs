...
internal static class Introspection
{
    ...
    public static Type InnerType(this Type type, Type generic)
    {
        ...
        return innerTypes.Length == 1
            ? innerTypes[0]
            : throw new InvalidOperationException($"Expected '{type.Name}' to have exactly one generic argument on the generic interface '{generic.Name}', but found {innerTypes.Length}.");
    }
    ...
    public static Boolean Implements<TInterface>(this Type type)
        where TInterface : class => type.Implements(typeof(TInterface));
