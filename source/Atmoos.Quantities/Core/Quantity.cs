        ...
        ICastOperators<Quantity, Double>
{
    ...
    public Quantity Project(in Measure other) => ReferenceEquals(this.measure, other) ? this : new Quantity(this.measure / (Polynomial)other * this.value, in other);

    private Double Project(in Quantity other) => ReferenceEquals(this.measure, other.measure) ? other.value : other.measure / (Polynomial)this.measure * other.value;
    ...
    public Boolean IsOf<TDimension>()
        where TDimension : IDimension => TDimension.D.Equals(this.measure.D);
