        ...
        IDivisionOperators<Dimension, Dimension, Dimension>
{
    ...
    public static Boolean operator ==(Dimension? left, Dimension? right) =>
        (left, right) switch {
            (null, null) => true,
...
internal abstract class Scalar : Dimension
{
    ...
    private sealed class Impl<T> : Scalar
        where T : IDimension
    {
        public Impl(Int32 multiplicity)
            : base(multiplicity) { }

        protected override Dimension Multiply(Dimension other) =>
            other switch {
                Unit => this,
...
internal sealed class Product : Dimension
{
    ...
    public override IEnumerator<Scalar> GetEnumerator() => WithMultiplicity(this.left, this.e).Concat(WithMultiplicity(this.right, this.e)).GetEnumerator();

    private static IEnumerable<Scalar> WithMultiplicity(Dimension dim, Int32 e) => e == 1 ? dim : dim.Select(d => d.Pow(e)).SelectMany(l => l);
...
file static class Convenience
{
    ...
    private static Dimension ReBuild(ReadOnlySpan<Dimension> values) =>
