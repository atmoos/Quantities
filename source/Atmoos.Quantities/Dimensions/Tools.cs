...
internal static class Tools
{
    ...
    public static IEnumerable<Type> Interfaces<TDimension>()
        where TDimension : IDimension => Interfaces(typeof(TDimension)).Distinct();

    private static IEnumerable<Type> Interfaces(Type type) =>
        [type.MostDerivedOf(dimension), .. type.InnerTypes(product).SelectMany(Unwrap), .. type.InnerTypes(genericDimension).SelectMany(Unwrap)];
