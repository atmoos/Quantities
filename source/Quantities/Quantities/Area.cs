    ...
    , IDivisionOperators<Area, Length, Length>
{
    ...
    public Area To<TArea>(in Scalar<TArea> other)
        where TArea : IArea, IAlias<ILength>, IUnit => new(other.Transform(in this.area, f => f.AliasOf<TArea, ILength>()));
    ...
    public static Area Of<TArea>(in Double value, in Scalar<TArea> measure)
        where TArea : IArea, IAlias<ILength>, IUnit => new(measure.Create(in value, f => f.AliasOf<TArea, ILength>()));
