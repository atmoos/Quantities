    ...
    , IDivisionOperators<Volume, Length, Area>
{
    ...
    public Volume To<TVolume>(in Scalar<TVolume> other)
        where TVolume : IVolume, IAlias<ILength>, IUnit => new(other.Transform(in this.volume, f => f.AliasOf<TVolume, ILength>()));
    ...
    public static Volume Of<TVolume>(in Double value, in Scalar<TVolume> measure)
        where TVolume : IVolume, IAlias<ILength>, IUnit => new(measure.Create(in value, f => f.AliasOf<TVolume, ILength>()));
